...
export function useAppState() {
  ...
  const initializeAnalyzer = useCallback(async (apiKey: string) => {
    ...
    try {
      dispatch({ type: 'SET_API_KEY_STATE', payload: 'uploading-system-prompt' });
      ...
      dispatch({ type: 'UPDATE_TOKEN_COUNTS', payload: { systemPrompt: systemPromptTokens } });
    } catch (err) {
      console.error('Failed to initialize analyzer:', err);
      dispatch({ type: 'SET_ERROR', payload: `Failed to initialize AI: ${(err as Error).message}` });
      dispatch({ type: 'SET_API_KEY_STATE', payload: 'missing' });
    }
  }, []);
  ...
  const updateLogFileTokens = useCallback(async () => {
    ...
    try {
      ...
      const logFileTokens = Math.max(0, totalTokens - state.tokenCounts.systemPrompt);
      
      dispatch({ type: 'UPDATE_TOKEN_COUNTS', payload: { logFile: logFileTokens } });
    } catch (err) {
  ...
  const actions = {
    ...
    uploadLogFile: useCallback(async (content: string, filename: string) => {
      if (!state.analyzer) {
        dispatch({ type: 'SET_ERROR', payload: 'AI not initialized. Please check your API key.' });
        return false;
      }
      ...
      try {
        ...
        dispatch({ type: 'SET_LOG_FILE_STATE', payload: 'attached' });
        
        return true;
      } catch (err) {
        dispatch({ type: 'SET_ERROR', payload: `Failed to process log file: ${(err as Error).message}` });
    ...
    sendMessage: useCallback(async (query: string) => {
      if (!selectors.canSendMessage(state)) {
        if (state.apiKeyState !== 'exists') {
          dispatch({ type: 'SET_ERROR', payload: 'Please configure your API key first' });
          dispatch({ type: 'SET_SETTINGS_OPEN', payload: true });
        } else if (state.logFileState !== 'attached') {
          dispatch({ type: 'SET_ERROR', payload: 'Please upload a log file first' });
        }
        return;
      }
      ...
      try {
        ...
        if (isFirstMessage) {
          ...
          console.log('Sending first message, creating chat session');
          console.log('Current state - logFileState:', state.logFileState, 'apiKeyState:', state.apiKeyState);
          ...
          console.log('Sending followup message to existing chat session');
          dispatch({ type: 'SET_UI_STATE', payload: 'waiting-for-ai-response' });
        ...
        for await (const chunk of stream) {
          fullResponse += chunk;
          dispatch({ type: 'UPDATE_CURRENT_RESPONSE', payload: fullResponse });
        }
        ...
        if (errorMessage.includes('context') || errorMessage.includes('limit') || errorMessage.includes('too large')) {
          dispatch({ 
            type: 'SET_ERROR', 
            payload: 'The conversation has exceeded the model\'s context limit. Please start a new conversation to continue.' 
          });
        } else {
