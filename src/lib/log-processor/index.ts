  ...
  constructor() {
    const transformer: Transformer<string, UnformattedLogInfo> = {
      transform(chunk, controller) {
        ...
        restOfFirstLine = restOfFirstLine.slice(labelEndIndex + 1).trim();
        ...
        const secondaryTags = restOfFirstLine.match(/\(([^)]+)\)$/)?.[1];
        if (secondaryTags) {
...
> {
  constructor() {
    function parseNestedStructure(lines: string[]): LogInfoPayload | undefined {
...
> {
  constructor() {
    ...
    function extractNodeIdTag(chunk: UnformattedLogInfo): number | undefined {
      const index = chunk.primaryTags?.findIndex((t) => t.startsWith("Node "));
    ...
    const transformer: Transformer<UnformattedLogInfo, SemanticLogInfo> = {
      transform(chunk, controller) {
        ...
        if (
          ...
          const rssi = extractAttribute<string>(chunk.message, "RSSI");

          let invalid = false;
          if (chunk.message.nested?.message.endsWith(" [INVALID]")) {
            invalid = true;
            chunk.message.nested.message = chunk.message.nested.message.slice(
              0,
              -" [INVALID]".length
            );
          }
        ...
        if (
          ...
          if (classified) {
            ...
            if (typeof stopWorryingTypeScript.message === "string") {
        ...
        if (
          ...
          if (chunk.primaryTags!.includes("+")) {
            const commandClass = chunk.primaryTags!.findLast((t) => t !== "+");
            ...
            const parsed = parseMetadataUpdatedMessage(chunk.message);
            if (parsed) {
