 ...
 * This can be used with different transports (e.g. stdio for CLI usage).
 */
export class ZWaveLogMCPServerCore {
	...
	/**
	 * Ensure the query engine is initialized, throwing an error if no log is loaded
	 */
	private ensureQueryEngine(): void {
		if (!this.queryEngine) {
			throw new McpError(
	...
	/**
	 * Load log file from file path
	 */
	public async loadLogFileFromPath(filePath: string): Promise<void> {
		const logContent = await readFile(filePath, "utf-8");
		await this.loadLogFileFromContent(logContent);
	}

	/**
	 * Load log file from content string
	 */
						...
						inputSchema: {
							...
							properties: {
								query: {
									type: "string",
									description:
										"Search query text or regex pattern. Searches ALL string-valued fields recursively throughout the log entry. For regex: wrap in forward slashes '/pattern/' or use regex syntax (|, *, +, ?, [], etc.) for auto-detection. Examples: 'battery', '/temp|humidity/', 'node.*[0-9]+'. Can be omitted if other filters are provided.",
								},
								entryKinds: {
									type: "array",
									description:
