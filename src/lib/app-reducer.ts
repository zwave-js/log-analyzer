	...
	switch (action.type) {
		case "SET_API_KEY": {
			const newState = {
				...
				apiKeyState: action.payload
					? ("uploading-system-prompt" as const)
					: ("missing" as const),
			};
		...
		case "SET_CURRENT_QUERY": {
			const newState = {
				...
				userQueryState: action.payload.trim()
					? ("not-empty" as const)
					: ("empty" as const),
			};
			...
			const userQueryTokens =
				action.payload.length > 0
					? Math.max(1, Math.floor(action.payload.length / 4))
					: 0;
			newState.tokenCounts = {
				...newState.tokenCounts,
				userQuery: userQueryTokens,
				total:
					newState.tokenCounts.systemPrompt +
					newState.tokenCounts.logFile +
					userQueryTokens,
			};
		...
		case "FINISH_RESPONSE": {
			...
			return {
				...
				tokenCounts: {
					...
					userQuery: 0,
					total:
						state.tokenCounts.systemPrompt +
						state.tokenCounts.logFile,
				},
			};
		}
		...
		case "UPDATE_TOKEN_COUNTS": {
			...
			newTokenCounts.total =
				newTokenCounts.systemPrompt +
				newTokenCounts.logFile +
				newTokenCounts.userQuery;
