...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const [greeting, setGreeting] = useState(() => getRandomGreeting(state.resetKey));
  ...
  }, [displayMessages.length, state.currentResponse, state.uiState, hasStartedChat]);
  ...
  return (
    ...
    >
      ...
      {hasStartedChat && (
        ...
        >
          {displayMessages.map((message, index) => {
            ...
            const isFirstUserMessage = message.type === 'user' && 
              displayMessages.slice(0, index + 1).filter(m => m.type === 'user').length === 1;
          ...
          {state.uiState === "waiting-for-ai-response" && (
            <>
              ...
              {state.isFirstResponse && state.firstResponseStartTime && (
                <FirstResponseIndicator startTime={state.firstResponseStartTime} />
              )}
            </>
          )}

          {/* New Chat Button - show after AI responses in chat area */}
          {hasStartedChat && state.uiState === "idle" && displayMessages.length > 0 && displayMessages[displayMessages.length - 1].type === "assistant" && (
      ...
      <Box
        sx={{
          position: inputBoxPosition === "center" ? "absolute" : "fixed",
        ...
        >
          ...
          <Box sx={{ px: 2, pb: 0 }}>
            <TextField
              ...
              placeholder={hasStartedChat ? "What else do you want to know?" : "Ask a question about your Z-Wave logs..."}
              variant="standard"
              multiline
              maxRows={6}
              fullWidth
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <AttachmentIndicator
                ...
                showAttachButton={!hasStartedChat && !state.hasChatSession}
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              {/* Send Button */}
              <Tooltip
                title={!canSendMessage ? getDisabledReason(state) : ""}
              ...
              >
                <span>
                  <IconButton
                    ...
                    sx={{
                      bgcolor: canSendMessage && state.tokenCounts.total <= 1000000 
                        ? "primary.main" 
                        : state.tokenCounts.total > 1000000 
                        ? "error.main"
                        : "transparent",
                      color: canSendMessage && state.tokenCounts.total <= 1000000
                      ...
                      "&:disabled": {
                        bgcolor: state.tokenCounts.total > 1000000 ? "error.main" : "transparent",
                        color: state.tokenCounts.total > 1000000 ? "error.contrastText" : "text.disabled",
                      },
                    }}
                  >
                    <SendIcon />
...
function getDisabledReason(state: ApplicationState): string {
  ...
  if (state.uiState === "waiting-for-ai-response" || state.uiState === "ai-responding") {
